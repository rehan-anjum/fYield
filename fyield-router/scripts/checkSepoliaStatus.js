...
async function main() {
    console.log("Checking Sepolia AAVE Manager Status");

    const [deployer] = await ethers.getSigners();

    const SEPOLIA_MANAGER_ADDRESS = process.env.SEPOLIA_MANAGER_ADDRESS;

    if (!SEPOLIA_MANAGER_ADDRESS) {
        console.error("Missing SEPOLIA_MANAGER_ADDRESS");
        process.exit(1);
    }

    const manager = await ethers.getContractAt("AAVEManager", SEPOLIA_MANAGER_ADDRESS);
